...
public class FactoryTrackerCommands(IDbContextFactory<SatisfactoryDbContext> dbContextFactory) : IFactoryTrackerCommands
{
    ...
    public async Task<MiningStation> AddMiningStationAsync(MiningStation station, CancellationToken cancellationToken = default)
    {
        ...
        var duplicate = await dbContext.MiningStations
            .AnyAsync(s => s.MineId == station.MineId && s.MinerMk == station.MinerMk && s.OverclockLevel == station.OverclockLevel && s.NodePurity == station.NodePurity, cancellationToken);
        if (duplicate)
            throw new InvalidOperationException("A mining station with this miner type, overclock level and node purity already exists for this mine.");
    ...
    public async Task UpdateMiningStationAsync(MiningStation station, CancellationToken cancellationToken = default)
    {
        ...
        var duplicate = await dbContext.MiningStations
            .AnyAsync(s => s.MineId == existing.MineId && s.MinerMk == station.MinerMk && s.OverclockLevel == station.OverclockLevel && s.NodePurity == station.NodePurity && s.Id != station.Id, cancellationToken);
        if (duplicate)
            throw new InvalidOperationException("A mining station with this miner type, overclock level and node purity already exists for this mine.");
        existing.MinerMk = station.MinerMk;
        existing.OverclockLevel = station.OverclockLevel;
        existing.Quantity = station.Quantity;
        existing.NodePurity = station.NodePurity;
        await dbContext.SaveChangesAsync(cancellationToken);
    }
