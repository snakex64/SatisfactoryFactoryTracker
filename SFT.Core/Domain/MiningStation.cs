...
public class MiningStation
{
    ...
    /// <summary>Number of power shards slotted: 0 = 100%, 1 = 150%, 2 = 200%, 3 = 250%.</summary>
    public int OverclockLevel { get; set; }
    public int Quantity { get; set; } = 1;
    /// <summary>Node purity for this miner's ore node: 0 = Impure (0.5×), 1 = Normal (1×), 2 = Pure (2×).</summary>
    ...
    /// </summary>
    public decimal ComputeOutputPerMinute()
    {
        var baseRate = MinerMk switch { 2 => 120m, 3 => 240m, _ => 60m };
        var purityMultiplier = NodePurity switch { 0 => 0.5m, 2 => 2.0m, _ => 1.0m };
        var overclockMultiplier = OverclockLevel switch { 1 => 1.5m, 2 => 2.0m, 3 => 2.5m, _ => 1.0m };
